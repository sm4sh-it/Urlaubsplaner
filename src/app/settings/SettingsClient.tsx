...
export default function SettingsClient({ initialProfiles }: { initialProfiles: Profile[] }) {
  ...
  const handleDelete = async (id: string, name: string) => {
    ...
    try {
      ...
      useStore.getState().addToast({
        ...
        description: `Profil "${name}" und zugehörige Daten wurden entfernt.`,
      })
      router.refresh()
    } catch (err: any) {
      ...
      useStore.getState().addToast({
        ...
        description: err.message || "Fehler beim Löschen des Profils.",
      })
    }
  }
  ...
  const handleSyncData = async () => {
    ...
    try {
      ...
      if (allOk) {
        const msg = `Feiertage & Ferien für ${statesToSync.length} Bundesländer (Jahr ${year}) erfolgreich synchronisiert!`
        setSyncFeedback(msg)
        useStore.getState().addToast({
          type: "success",
          title: "Kalenderdaten synchronisiert",
          description: msg,
        })
      } else {
        ...
        useStore.getState().addToast({
          ...
          description: "Einige Daten konnten nicht vollständig geladen werden.",
        })
      }
      ...
      useStore.getState().addToast({
        type: "error",
        title: "Fehler bei der Synchronisation",
        description: err.message || "Die Feiertage & Ferientermine konnten nicht geladen werden.",
      })
      setTimeout(() => setSyncFeedback(null), 5000)
    } finally {
      setIsSyncing(false)
    ...
    <div className="flex flex-col gap-8 pb-12">
      ...
      {profiles.length === 0 ? (
        <div className="bg-white dark:bg-[#0d141d]/75 backdrop-blur-xl border border-slate-200 dark:border-white/10 rounded-2xl sm:rounded-3xl p-12 text-center flex flex-col items-center justify-center shadow-xs">
          ...
          <button
            onClick={handleOpenAdd}
            className="mt-6 btn-glass inline-flex items-center gap-2 font-semibold text-xs sm:text-sm text-slate-700 dark:text-slate-200"
          >
            <UserPlus className="w-4 h-4 text-brand-500" />
                ...
                <div>
                  ...
                  <div className="flex items-start justify-between gap-3 mb-4">
                    <div className="flex items-center gap-3.5 min-w-0">
                      <div
                        className="w-11 h-11 rounded-2xl flex items-center justify-center text-white text-base font-bold shadow-md shrink-0"
                    ...
                    <div className="flex items-center gap-1 shrink-0">
                      <button
                        ...
                        className="p-2 rounded-xl text-slate-400 hover:text-brand-600 dark:hover:text-brand-400 hover:bg-slate-100 dark:hover:bg-white/10 transition-colors cursor-pointer"
                  ...
                  <div className="grid grid-cols-2 gap-3 my-4 bg-slate-50/80 dark:bg-[#070c12]/60 border border-slate-200/80 dark:border-white/10 rounded-2xl p-3.5">
                    <div>
                      <span className="block text-[11px] font-semibold text-slate-400 dark:text-slate-500 uppercase tracking-wider truncate">
                        Jahresurlaub
                      </span>
                      <div className="flex items-baseline gap-1.5 mt-0.5 flex-wrap">
                        ...
                        {p.additionalLeave > 0 && (
                          <span
                            className="text-[11px] text-brand-600 dark:text-brand-400 font-semibold font-mono"
                            title={`${p.additionalLeave} Tage Zusatzurlaub`}
                          >
                            (+{p.additionalLeave} Zusatz)
                          </span>
                        )}
                      </div>
                    </div>
      ...
      <div className="bg-white dark:bg-[#0d141d]/75 backdrop-blur-xl border border-slate-200 dark:border-white/10 rounded-2xl sm:rounded-3xl p-5 sm:p-6 shadow-sm flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6 mt-4">
        <div className="flex items-start gap-4">
          ...
          <div>
            ...
            {syncFeedback && (
              <span className="inline-flex items-center gap-1.5 text-xs font-semibold text-emerald-600 dark:text-emerald-400 mt-2 font-mono">
                <Check className="w-4 h-4" />
                {syncFeedback}
              </span>
            )}
        ...
        <button
          ...
          className="inline-flex items-center justify-center gap-2 px-5 py-2.5 rounded-xl font-bold text-xs sm:text-sm text-slate-700 dark:text-slate-200 bg-slate-100 hover:bg-slate-200/80 dark:bg-[#070c12]/80 dark:hover:bg-[#161f28] hover:text-brand-600 dark:hover:text-brand-400 border border-slate-200 dark:border-white/10 transition-all cursor-pointer disabled:opacity-50 shrink-0 shadow-xs"
        >
