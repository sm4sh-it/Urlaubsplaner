 ...
 * Unterstützt moderne Browser (Clipboard API) sowie Fallbacks für
 * ungesicherte HTTP-Kontexte (z. B. Docker LAN-Zugriff über IP:Port) und ältere Browser.
 */
export async function copyToClipboard(text: string): Promise<boolean> {
  // 1. Moderne Clipboard API (verfügbar in HTTPS oder localhost)
  if (
    ...
    try {
      ...
      console.warn("navigator.clipboard.writeText fehlgeschlagen, verwende Fallback:", err)
    }
  }

  // 2. Fallback: Unsichtbares textarea mit document.execCommand('copy')
  // Funktioniert in allen Browsern auch über reines HTTP / Docker LAN IP
  if (typeof document !== "undefined") {
    try {
      const textArea = document.createElement("textarea")
      textArea.value = text

      // Verhindert Scrollen oder Layout-Verschiebungen
      ...
      console.error("Fallback execCommand copy fehlgeschlagen:", err)
      return false
    }
  }
